...
import type { TriggerNodeData, TelemetryTriggerConfig } from "../types/flow-types";
...
import { queryLogs, queryMetrics, type LogData, type MetricData } from "@/api/telemetry";
  ...
  const initialState = useMemo(
    () => ({
      triggerType: (nodeData?.triggerType ||
        "manual") as "manual" | "schedule" | "webhook" | "telemetry",
  ...
  const handleSave = () => {
    ...
    if (state.triggerType === "telemetry") {
      ...
      if (state.telemetryType === "log") {
        if (state.severity) telemetryConfig.severity = state.severity as "error" | "warn" | "info";
    ...
    const updatedData: TriggerNodeData = {
      ...
      telemetryConfig: state.triggerType === "telemetry" ? telemetryConfig : undefined,
    };
            ...
            <>
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => updateField("operator", value as ">" | "<" | ">=" | "<=")}
                >
                  <SelectTrigger className="h-9">
                    <SelectValue />
                  </SelectTrigger>
      ...
      {state.triggerType === "telemetry" && (
        <div className="space-y-3 border-t pt-4">
          ...
          {state.telemetryType === "log" && previewLogs.length > 0 && (
            <div className="space-y-2 rounded-md border p-3">
              <div className="text-sm font-medium">
                Found {previewLogs.length} matching log{previewLogs.length !== 1 ? "s" : ""}
              </div>
          ...
          {state.telemetryType === "metric" &&
            ...
            !previewError && (
              <div className="rounded-md border p-3 text-sm text-muted-foreground">
                No matching metrics found exceeding threshold in the last 24 hours
              </div>
            )}
        </div>
      )}
