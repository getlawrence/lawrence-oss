          ...
          <TableBody>
            {data.map((result, idx) => {
              ...
              const metricName =
                ...
                result.labels?.name ||
                result.data?.name ||
                (result.type === "metrics" ? "metric" : result.type === "logs" ? "log" : "trace");
              ...
              return (
                <TableRow key={idx}>
                  ...
                  <TableCell className="w-48 font-mono text-sm font-medium truncate" title={String(metricName)}>
                    {String(metricName)}
                  </TableCell>
                  ...
                  <TableCell className="w-80">
                    <div className="flex flex-wrap gap-1 max-h-20 overflow-y-auto">
                      {otherLabels.map(([key, value]) => (
                        <Badge key={key} variant="secondary" className="text-xs whitespace-nowrap">
                          {key}={value}
                        </Badge>
                      ))}
                    </div>
